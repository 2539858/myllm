...
def precompute_freqs_cis(dim: int, end: int = int(32 * 1024), rope_base: float = 1e6,
    ...
    # 这里添加了YaRN的部分，课上没有讲这部分，可以结合picture中yarn部分理解
...
def repeat_kv(x: torch.Tensor, n_rep: int) -> torch.Tensor:
    batch_size, seq_length, num_key_value_heads, head_dim = x.shape
    if n_rep == 1:
        return x
    return (
        x[:, :, :, None, :].expand(batch_size, seq_length, num_key_value_heads, n_rep, head_dim
                                   ).reshape(batch_size, seq_length, num_key_value_heads * n_rep, head_dim)
    )
