    ...
    def forward(self,
        ...
        if self.flash and seq_len > 1 and (attention_mask is None or torch.all(attention_mask == 1)):
            ...
            scores = scores + torch.triu(
                ...
                diagonal=1
            ).unsqueeze(0).unsqueeze(0)  # scores+mask 屏蔽上三角的值
    ...
    def forward(self, hidden_states):
        ...
        if self.training and self.alpha > 0.0:
            ...
            # 序列级均衡
                ...
                aux_loss = (Pi * fi).sum() * self.alpha # Pi和fi都均匀分布时，这个值最小 Pi打分 fi实际
        else:
            aux_loss = 0
...
class MOEFeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            x = x.repeat_interleave(self.config.num_experts_per_tok, dim=0) # 复制top_k
...
class MiniMindBlock(nn.Module):
    ...
    def forward(self, hidden_states, position_embeddings, past_key_value=None, use_cache=False, attention_mask=None):
        ...
        hidden_states += residual # 把注意力层学到的新信息加到旧信息上
