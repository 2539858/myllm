...
# MoE模型需使用此函数转换
def convert_torch2transformers_myllm(torch_path, transformers_path, dtype=torch.float16):
    Config.register_for_auto_class()
    ForCausalLM.register_for_auto_class("AutoModelForCausalLM")
    lm_model = ForCausalLM(lm_config)
    ...
    print(f"模型已保存为 Transformers-Myllm 格式: {transformers_path}")
...
if __name__ == '__main__':
    lm_config = Config(hidden_size=512, num_hidden_layers=8, max_seq_len=8192, use_moe=False)
